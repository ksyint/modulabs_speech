...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.conv1 = nn.Conv2d(3, 16, kernel_size=7, padding=3, # 128
                               bias=False)
        self.layer1 = self._make_layer(block, 16, layers[0]) # 128
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2) # 64
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2) # 32
        self.bn1 = nn.BatchNorm2d(64*block.expansion) # N*256*32*32
        self.relu = nn.ReLU(inplace=True)
