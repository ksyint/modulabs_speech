...
def cut_patch(img, landmarks, height, width, threshold=5):
    ...
    cropped_landmarks = landmarks - [int(round(center_x - width)), int(round(center_y - height))]
    return cutted_img, cropped_landmarks
...
def crop_patch(video_pathname, landmarks, mean_face_landmarks, stablePntsIDs, STD_SIZE, window_margin, start_idx, stop_idx, crop_height, crop_width):
    ...
    while True:
        ...
        if len(q_frame) == margin:
            ...
            # -- crop mouth patch
            crop_image, crop_landmarks = cut_patch( trans_frame,
                                        trans_landmarks[start_idx:stop_idx],
                                        crop_height//2,
                                        crop_width//2,)
            sequence.append(crop_image)
            lm_sequence.append(crop_landmarks)
        if frame_idx == len(landmarks)-1:
            while q_frame:
                ...
                sequence.append(crop_image)
                lm_sequence.append(crop_landmarks)
            return np.array(sequence), np.array(lm_sequence)
        frame_idx += 1
    return None, None
...
def bbox_interpolate(bboxes):

    valid_frames_idx = [idx for idx, bbox in enumerate(bboxes) if len(bbox) != 0]
    if not valid_frames_idx:
        return None
    for idx in range(1, len(valid_frames_idx)):
        if valid_frames_idx[idx] - valid_frames_idx[idx-1] == 1:
            ...
            bboxes = linear_interpolate(bboxes, valid_frames_idx[idx-1], valid_frames_idx[idx])
    valid_frames_idx = [idx for idx, bbox in enumerate(bboxes) if len(bbox) != 0]
    # -- Corner case: keep frames at the beginning or at the end failed to be detected.
    if valid_frames_idx:
        bboxes[:valid_frames_idx[0]] = [bboxes[valid_frames_idx[0]]] * valid_frames_idx[0]
        bboxes[valid_frames_idx[-1]:] = [bboxes[valid_frames_idx[-1]]] * (len(bboxes) - valid_frames_idx[-1])
    valid_frames_idx = [idx for idx, bbox in enumerate(bboxes) if len(bbox) != 0]
    assert len(valid_frames_idx) == len(bboxes), "not every frame has landmark"
    return bboxes
